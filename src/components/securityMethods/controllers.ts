...
import { create_PinsCode, create_SecretQuestions } from "./services";
...
export const set_pinCode = async (req: Request, res:Response, next:NextFunction)=>{
    ...
    if(pins !== "Pins Created"){
        return res.status(404).json({message: "User Already has Pins"});
    }
...
export const set_secretQuestions = async (req: Request, res:Response)=>{
    ...
    if(secretQTS !== "Questions created"){
        return res.status(404).json({message: "User already has Secret questions"});
    }

    return res.status(201).json({message: "Questions Created"});
}